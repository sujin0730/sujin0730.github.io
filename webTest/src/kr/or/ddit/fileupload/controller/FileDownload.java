...
public class FileDownload extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 저장될 폴더가 없으면 새로 만들어 놓는다(폴더가 없으면 오류 나므로 생성해야함)
		...
		// 다운받을 파일의 File객체 생성 → 설제 저장된 파일을 저장한다.
		...
		if(downFile.exists()) {	//해당 파일이 있으면...
			...
			// filename은 원래의 파일명으로 지정한다.
			// String headerValue = "attachment; filename=\""+fvo.getOrigin_file_name()+"\"";
			String headerValue = "attachment; "+ getEncodedFileName(request, fvo.getOrigin_file_name());	// 한글로저장된 파일을 다운로드하기 위함
	...
	// 다운로드파일의 파일명이 한글일 때 한글이 깨지는것을 방지하는 메소드
