...
/*
	- Servlet 3.0이상에서 파일 업로드를 하려면 서블릿 @MultipartConfig 애노테이션을 설정해야 한다.
	...
	4) maxRequestSize : 전체파일크기와 파일 이외의 데이터 크기의 합계의 최대크기(단위 : byte, 기본값 -1L(무제한))
	
	
 */

@MultipartConfig(
		fileSizeThreshold = 1024 * 1024 * 10, //10Mbyte
...
public class FileUpload extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//GET방식으로 요청하면 FileUpload 폼이 있는 jsp 문서로 포워딩한다.
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//-------------------------------------------------------------------
		// 파일이 아닌 일반데이터는 getParameter() 메서드나 getParameterValues()메서드를 
		// 이용해서 구한다.
		...
		// 업로드된 파일 목록이 저장될 List 객체 생성
		List<FileInfoVO> fileList = new ArrayList<FileInfoVO>();
		
		/*
			- Servlet 3.0이상에서 파일 업로드와 관련되어 추가된 메서드
		...
		for(Part part : request.getParts()) {
			...
			if(!"".equals(fileName)) {	// 파일인지 검사
				...
				// 실제 저장된는 파일이름이 중복되는 것을 방지하기 위해서 
				...
				try {
					// 파일을 저장한다. → Part객체의 write()메서드 이용
					// 형식) Part객체.write("저장될경로/저장될파일명")
	...
	/*
		...
		------------------------------dsfwserwdf131esdf 	→ 각 Part를 구분하는 구분선
