...
/*
	...
	@WebServlet 애노테이션 속성들
	1. name : 서블릿의 이름을 설정한다.( 기본값 : 빈문자열("") )
	2. urlPatterns : 서블릿의 URL패턴의 목록을 설정한다.( 기본값 : 빈배열({ }) )
		예) urlPatterns = "/url1" 또는 urlPatterns={"/url"} → url패턴이 1개일 경우(중괄호 생략가능)
		예) urlPatterns = {"/url1", "/url2", ...} → url패턴이 2개 이상일 경우
...
public class ServletTest02 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<html><head><meta charset='utf-8'><title>두번째 Servlet 연습</title></head>");
		out.println("<body>");
